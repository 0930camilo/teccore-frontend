...
export class AlumnoListPageComponent implements OnInit {
  ...
  protected readonly descripcion = 'Consulta y busqueda de alumnos registrados.';
  protected readonly loadFn = (filtros: PaginacionRequest) => this.service.listar(filtros);

  protected readonly createFn = (payload: Record<string, unknown>): Observable<ApiResponse<unknown>> => {
  ...
  protected createFields: FormField[] = [
    ...
    { key: 'semestreId', label: 'Semestre', type: 'select', required: false, options: [] },
    { key: 'materiaIds', label: 'Materias adicionales', type: 'multiselect', required: false, options: [] }
  ];
  ...
  private loadSemestres(): void {
    this.semestreService.listar({ page: 0, size: 100 }).subscribe({
      next: (response) => {
        const semestres = this.extractRecords(response.data);
        this.updateFieldOptions(
          'semestreId',
          semestres
            .filter((semestre): semestre is Semestre & { id: number } => semestre.id != null)
            .map((semestre) => ({
              value: semestre.id,
  ...
  private loadMaterias(): void {
    this.materiaService.listar({ page: 0, size: 500 }).subscribe({
      next: (response) => {
        const materias = this.extractRecords(response.data);
        this.updateFieldOptions(
          'materiaIds',
          materias
            .filter((materia): materia is Materia & { id: number } => materia.id != null)
            .map((materia) => ({
              value: materia.id,
  ...
  private updateFieldOptions(key: string, options: { value: unknown; label: string }[]): void {
    this.createFields = this.createFields.map((field) =>
      field.key === key ? { ...field, options } : field
    );
  }

  private extractRecords<T>(data: PaginacionRespuesta<T> | T[] | null | undefined): T[] {
    if (Array.isArray(data)) {
      return data;
    }

    return (data?.content ?? data?.items ?? data?.data ?? []) as T[];
  }
  ...
  private formatMateriaOption(materia: Materia): string {
    const nombre = materia.nombre || `Materia #${materia.id}`;
    const semestre = materia.semestreNombre || (materia.semestre as { nombre?: string } | undefined)?.nombre;
    return semestre ? `${nombre} - ${semestre}` : nombre;
  }
}
